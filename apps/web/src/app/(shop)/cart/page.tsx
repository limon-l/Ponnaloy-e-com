        ...
        <div className="lg:col-span-2 space-y-4">
          {items.map((item) => (
            <div key={item.id} className="flex gap-4 p-4 border rounded-lg">
              ...
              <div className="text-right font-semibold shrink-0">{formatPrice(item.price * item.quantity)}</div>
            </div>
          ))}
          <Button variant="ghost" asChild className="mt-4">
            <Link href="/products"><ArrowLeft className="h-4 w-4 mr-2" />Continue Shopping</Link>
          </Button>
        </div>
