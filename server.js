...
function buildStoreFacts(products) {
  ...
  const topRated = [...products].sort((left, right) => right.rating - left.rating || left.price - right.price).slice(0, 4);
  const bestValue = [...products].sort((left, right) => left.price - right.price || right.rating - left.rating).slice(0, 4);
...
function buildAssistantReply(message, matches, products) {
  ...
  const primaryMatches = primaryQuery ? findTopProducts(products.filter((p) => p.id !== explicitMention?.id), primaryQuery, 3) : matches;

  if (compareMode) {
    const left = primaryMatches[0] || explicitMention || matches[0] || products[0];
    const right = primaryMatches.find((p) => p.id !== left?.id) || matches.find((p) => p.id !== left?.id) || products.find((p) => p.id !== left?.id);
    if (left && right) {
      const comparison = compareProducts(left, right);
      return { reply: comparison.summary, tone: "comparison", focusCategory: left.category, followUp: "You can compare another pair or ask for a recommendation by use case.", comparison };
    }
  }
  ...
  if (recommendationMode && matches.length > 0) {
    ...
    return {
      ...
      followUp: secondary ? `An alternative is ${secondary.name} (${secondary.rating.toFixed(1)} stars, ${fmt(secondary.price)}).` : "Specify your budget or use case for a more targeted recommendation.",
      comparison: null,
    };
  }
...
app.get("/api/products/:identifier", asyncHandler(async (req, res) => {
...
app.post("/api/orders", requireAuth, asyncHandler(async (req, res) => {
  ...
  if (!customerName || !shippingAddress || !phone || !email) return res.status(400).json({ message: "Checkout details are incomplete." });
  const order = await createOrder({ userId: req.session.userId, items, customerName, shippingAddress, phone, email });
...
app.post("/api/chat", asyncHandler(async (req, res) => {
  ...
  if (!openai) {
    ...
    return res.json({
      ...
      storeFacts: { productCount: storeFacts.productCount, categoryCount: storeFacts.categoryCount, categories: storeFacts.categories },
      highlights: storeFacts.topRated.slice(0, 3), valuePicks: storeFacts.bestValue.slice(0, 3),
      matches: matches.length > 0 ? matches : storeFacts.topRated.slice(0, 4), comparison: assistant.comparison,
    });
  }
  ...
  const topProducts = matches.length >= 3 ? matches : [...matches, ...storeFacts.topRated.filter((p) => !matches.find((m) => m.id === p.id))].slice(0, 6);
  const productCatalog = topProducts.map((p) => `- ${p.name} (id:${p.id}) | category: ${p.category} | price: $${p.price} | rating: ${p.rating}/5 | stock: ${p.stock} | badge: ${p.badge || "none"} | features: ${p.shortDescription}`).join("\n");
...
- Always end with a brief follow-up question to keep the conversation going.`;
  ...
  try {
    ...
    if (req.session.chatHistory.length > 20) req.session.chatHistory = req.session.chatHistory.slice(-20);
    const tone = mention ? "product" : /\b(compare|versus|vs|difference)\b/i.test(rawMessage) ? "comparison" : /\b(recommend|suggest|best|top|good for)\b/i.test(rawMessage) ? "recommendation" : /\b(hi|hello|hey|thanks)\b/i.test(rawMessage) ? "greeting" : "product";
    return res.json({
      reply, tone, focusCategory: mention?.category || (matches[0]?.category ?? "General"),
      followUp: "Is there anything else you'd like to know?",
      storeFacts: { productCount: storeFacts.productCount, categoryCount: storeFacts.categoryCount, categories: storeFacts.categories },
      highlights: storeFacts.topRated.slice(0, 3), valuePicks: storeFacts.bestValue.slice(0, 3),
      matches: matches.length > 0 ? matches : storeFacts.topRated.slice(0, 4), comparison: null,
    });
  } catch (error) {
